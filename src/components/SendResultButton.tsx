...
export default function SendResultButton({ studentId, studentName, results, className }: SendResultButtonProps) {
  ...
  return (
    <div className={`relative ${className || ''}`} ref={dropdownRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 bg-white rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 w-72 p-4 animate-scale-in dark:bg-slate-800">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">Send Results</h3>
            <button onClick={() => setOpen(false)} className="p-1 hover:bg-slate-100 dark:bg-slate-700 rounded dark:hover:bg-slate-700">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border transition-colors ${
                mode === 'whatsapp'
                  ...
                  : 'bg-white text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700'
              }`}
            >
