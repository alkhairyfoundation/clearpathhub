...
export default function SendResultButton({ studentId, studentName, results, className }: SendResultButtonProps) {
  ...
  return (
    <div className={`relative ${className || ''}`} ref={dropdownRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 bg-white rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 w-72 p-4 animate-scale-in">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">Send Results</h3>
            <button onClick={() => setOpen(false)} className="p-1 hover:bg-slate-100 dark:bg-slate-700 dark:bg-slate-700 rounded">
              <X size={16} className="text-slate-400 dark:text-slate-500 dark:text-slate-500" />
            </button>
          </div>

          <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-3">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border transition-colors ${
                mode === 'whatsapp'
                  ...
                  : 'bg-white text-slate-600 dark:text-slate-400 dark:text-slate-400 border-slate-200 dark:border-slate-700 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-800 dark:bg-slate-800'
              }`}
            >
          ...
          {error && (
            <div className="text-xs text-red-600 dark:text-red-400 dark:text-red-400 bg-red-50 dark:bg-red-900/20 dark:bg-red-900/20 rounded-lg p-2 mb-2">{error}</div>
          )}
          ...
          {results.length > 0 && (
            <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 dark:border-slate-700">
              <p className="text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-500 mb-1.5">Results included:</p>
              <div className="space-y-0.5">
                {results.slice(0, 8).map((r, i) => (
                  <div key={i} className="flex justify-between text-[10px]">
                    <span className="text-slate-600 dark:text-slate-400 dark:text-slate-400 truncate mr-2">{r.subject_name}</span>
                    <span className={`font-medium ${r.score >= 70 ? 'text-green-600 dark:text-green-400 dark:text-green-400' : r.score >= 50 ? 'text-amber-600 dark:text-amber-400 dark:text-amber-400' : 'text-red-600 dark:text-red-400 dark:text-red-400'}`}>{r.score}%</span>
                  </div>
                ))}
                {results.length > 8 && (
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-500">+{results.length - 8} more</p>
                )}
              </div>
            </div>
          )}
