...
export default function TeacherClassesPage() {
  ...
  return (
    <DashboardLayout title="My Classes" subtitle="Manage your classes and view student rosters">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">My Classes</h1>
              <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400">Manage your classes and view student rosters</p>
            </div>
          </div>
      ...
      <div className="card">
        <div className="relative mb-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 dark:text-slate-500" size={18} /><input type="text" placeholder="Search classes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="input pl-10" /></div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16"><GraduationCap className="mx-auto text-slate-300 mb-4" size={48} /><p className="font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400">No classes found</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(cls => {
              ...
              return (
                <div key={cls.id} className="border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                  <div className="p-5 cursor-pointer" onClick={() => setSelectedClass(isSelected ? null : cls.id)}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 dark:bg-blue-900/30 rounded-lg flex items-center justify-center"><GraduationCap size={24} className="text-blue-600 dark:text-blue-400 dark:text-blue-400" /></div>
                      <div className="flex gap-1">
                        <button onClick={(e) => { e.stopPropagation(); openModal(cls); }} className="p-1.5 hover:bg-slate-100 dark:bg-slate-700 dark:bg-slate-700 rounded-lg"><Edit size={14} className="text-slate-600 dark:text-slate-400 dark:text-slate-400" /></button>
                        <button onClick={(e) => { e.stopPropagation(); handleDelete(cls.id); }} className="p-1.5 hover:bg-red-50 dark:bg-red-900/20 dark:bg-red-900/20 rounded-lg"><Trash2 size={14} className="text-red-600 dark:text-red-400 dark:text-red-400" /></button>
                      </div>
                    </div>
                    <h3 className="font-bold text-slate-900 dark:text-white dark:text-white text-lg">{cls.name}</h3>
                    {cls.section && <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">Section: {cls.section}</p>}
                    <div className="flex items-center justify-between mt-3">
                      <span className="flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400"><Users size={14} />{classStudents.length} students</span>
                      <span className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-500">{cls.capacity ? `${classStudents.length}/${cls.capacity}` : ''}</span>
                    </div>
                  </div>
                  {isSelected && classStudents.length > 0 && (
                    <div className="border-t border-slate-200 dark:border-slate-700 dark:border-slate-700 p-4 bg-slate-50 dark:bg-slate-800 dark:bg-slate-800">
                      <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 dark:text-slate-200 mb-2">Student Roster</h4>
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {classStudents.map(s => (
                          <div key={s.id} className="flex items-center justify-between text-sm">
                            <span>{s.profile?.first_name} {s.profile?.last_name}</span>
                            <span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 font-mono">{s.admission_number}</span>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto animate-scale-in">
            <div className="p-5 border-b border-slate-200 dark:border-slate-700 dark:border-slate-700 flex items-center justify-between sticky top-0 bg-white z-10 rounded-t-2xl"><h3 className="text-lg font-bold text-slate-900 dark:text-white dark:text-white">{editingClass ? 'Edit' : 'New'} Class</h3><button onClick={() => setShowModal(false)} className="p-1.5 hover:bg-slate-100 dark:bg-slate-700 dark:bg-slate-700 rounded-lg"><X size={20} className="text-slate-500 dark:text-slate-400 dark:text-slate-400" /></button></div>
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-slate-200 dark:border-slate-700 dark:border-slate-700 bg-white sticky bottom-0"><button onClick={() => setShowModal(false)} className="btn-ghost">Cancel</button><button onClick={handleSave} disabled={saving} className="btn-primary">{saving ? 'Saving...' : editingClass ? 'Update' : 'Create'}</button></div>
          </div>
        </div>
        )}
      </div>
