...
export default function TeacherTasksPage() {
  ...
  const taskTypeIcons: Record<string, { bg: string; color: string }> = {
    reading: { bg: 'bg-blue-100', color: 'text-blue-600' },
    study: { bg: 'bg-purple-100', color: 'text-purple-600' },
    project: { bg: 'bg-green-100', color: 'text-green-600' },
    research: { bg: 'bg-amber-100', color: 'text-amber-600' },
    other: { bg: 'bg-slate-100', color: 'text-slate-600' },
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500">View, submit, and track tasks assigned by the admin</p>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex gap-1 mb-6 bg-slate-100 rounded-lg p-1">
            {(['pending', 'submitted', 'graded'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} className={`flex-1 px-4 py-2 rounded-md text-sm font-semibold transition-all capitalize ${tab === t ? 'bg-white text-primary-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                {t} ({counts[t]})
              </button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-slate-500"><BookOpen size={48} className="mx-auto mb-4 opacity-50" /><p>No {tab} tasks</p></div>
          ) : (
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 capitalize">{task.task_type}{task.due_date ? ` • Due ${new Date(task.due_date).toLocaleDateString()}` : ''}</p>
                        </div>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${task.status === 'graded' ? 'bg-green-100 text-green-700' : task.status === 'submitted' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>{task.status}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {task.status === 'graded' && task.admin_grade !== null && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-primary-600">{task.admin_grade}/100</span>
                        </div>
                      )}
                      {task.feedback && <span className="text-xs text-slate-500">Feedback: {task.feedback}</span>}
                    </div>
                  </div>
                );
              })}
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full">
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  {selectedTask.description && <p className="text-sm text-slate-600 mt-1">{selectedTask.description}</p>}
                  {selectedTask.due_date && <p className="text-xs text-slate-400 mt-1 flex items-center gap-1"><Calendar size={12} />Due: {new Date(selectedTask.due_date).toLocaleDateString()}</p>}
                </div>
                <div>
