...
export default function TeacherResultsPage() {
  ...
  return (
    <DashboardLayout title="Results & Analytics" subtitle="Teacher - Compiled test performance analytics per student">
          ...
          <div>
            ...
            >
              ...
              {filteredStudents.map(st => (
                <option key={st.profile_id} value={st.profile_id}>{st.name} ({st.admission_number || 'N/A'})</option>
              ))}
            </select>
          </div>
        </div>
                ...
                {data.scoreTrend.length >= 2 && (
                  <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      <LineChart data={data.scoreTrend.map(t => ({ name: t.testTitle.substring(0, 20), score: t.score }))}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {data.insights.strengths.length > 0 && (
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
                      ...
                      <ul className="space-y-2">
                        {data.insights.strengths.map(st => (
                          <li key={st} className="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300">
                            <CheckCircle size={16} className="text-green-500 mt-0.5 flex-shrink-0" />
                            <span>{st} — performing well above average.</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {data.insights.needsImprovement.length > 0 && (
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
                      ...
                      <ul className="space-y-2">
                        {data.insights.needsImprovement.map(st => (
                          <li key={st} className="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300">
                            <XCircle size={16} className="text-red-500 mt-0.5 flex-shrink-0" />
                            <span>{st} — needs focused attention.</span>
                          </li>
                        ))}
                      </ul>
                    </div>
