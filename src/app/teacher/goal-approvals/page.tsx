    ...
    <DashboardLayout title="Goal Approvals" subtitle="Review and approve student growth goals">
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white dark:text-white">Goal Approvals</h1>
            <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-1">Review student growth goals for your class</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="card">
              <h2 className="font-semibold text-slate-900 dark:text-white dark:text-white mb-4 flex items-center gap-2">
                <Users size={18} /> Pending Approvals ({pendingGoals.length})
              </h2>
              {pendingGoals.length === 0 ? (
                <p className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-500 py-4 text-center">No pending approvals</p>
              ) : (
                <div className="space-y-3">
                  {pendingGoals.map((goal) => (
                    <div key={goal.id} className="flex items-center justify-between p-4 bg-amber-50 dark:bg-amber-900/20 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-900/40 dark:border-amber-900/40">
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-slate-900 dark:text-white dark:text-white">{goal.student?.first_name} {goal.student?.last_name}</p>
                        <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-0.5">
                          {goal.archetype?.name} &middot; {goal.goal_skills?.length || 0} skills
                        </p>
                        <p className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-500 mt-0.5">{new Date(goal.submitted_at).toLocaleDateString()}</p>
                      </div>
            ...
            {otherGoals.length > 0 && (
              <div className="card">
                <h2 className="font-semibold text-slate-900 dark:text-white dark:text-white mb-4">Previous Approvals ({otherGoals.length})</h2>
                <div className="space-y-2">
                  {otherGoals.map((goal) => (
                    <div key={goal.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 dark:bg-slate-800 rounded-lg">
                      <div>
                        <p className="text-sm font-medium text-slate-900 dark:text-white dark:text-white">{goal.student?.first_name} {goal.student?.last_name}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">{goal.archetype?.name}</p>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        goal.status === 'active' ? 'bg-emerald-100 dark:bg-emerald-900/30 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 dark:text-emerald-300' : 'bg-slate-100 dark:bg-slate-700 dark:bg-slate-700 text-slate-500 dark:text-slate-400 dark:text-slate-400'
                      }`}>{goal.status}</span>
                    </div>
                  ))}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div className="bg-primary-50 dark:bg-primary-900/20 dark:bg-primary-900/20 rounded-xl p-4">
                <p className="font-semibold text-primary-800 dark:text-primary-200 dark:text-primary-200">{selectedGoal.student?.first_name} {selectedGoal.student?.last_name}</p>
                <p className="text-sm text-primary-600 dark:text-primary-400 dark:text-primary-400 mt-1">{selectedGoal.archetype?.name}</p>
                <p className="text-sm text-primary-700 dark:text-primary-300 dark:text-primary-300 italic mt-2">{selectedGoal.goal_statement_snapshot}</p>
              </div>

              <div>
                <label className="label">Set Initial Rubric Levels</label>
                <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-3">Set the starting level for each selected skill.</p>
                {selectedGoal.goal_skills?.map((gs: any) => (
                  <div key={gs.id} className="flex items-center gap-3 mb-2">
                    <span className="text-sm text-slate-700 dark:text-slate-300 dark:text-slate-300 w-36">{gs.skill?.name}</span>
                    <select
