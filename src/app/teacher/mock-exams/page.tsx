        ...
        {view === 'exams' && (
          <>
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-900">Available Mock Exams</h2>
            </div>
            {exams.length === 0 ? (
              <div className="card text-center py-16">
                <GraduationCap size={48} className="mx-auto text-slate-300 mb-4" />
                <p className="text-slate-500">No mock exams available yet.</p>
              </div>
            ) : (
        ...
        {view === 'analytics' && selectedExam && !selectedStudent && (
          <>
            <div className="flex items-center gap-4 mb-4">
              <button onClick={() => { setView('exams'); setSelectedExam(null); }} className="p-2 hover:bg-slate-100 rounded-lg">
                <ArrowLeft size={20} className="text-slate-600" />
              </button>
              <div>
                <h2 className="text-xl font-bold text-slate-900">{selectedExam.title}</h2>
                <p className="text-sm text-slate-500">{selectedExam.exam_type === 'JSS3_BECE' ? 'JSS3 BECE' : 'SS3 WAEC'} — Class Analytics</p>
              </div>
            </div>

            {summary && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="card"><p className="text-xs text-slate-500">Total Students</p><p className="text-2xl font-bold">{summary.total}</p></div>
                <div className="card"><p className="text-xs text-slate-500">Average Score</p><p className="text-2xl font-bold text-blue-600">{summary.averageScore}%</p></div>
                <div className="card"><p className="text-xs text-slate-500">Best Score</p><p className="text-2xl font-bold text-green-600">{summary.bestScore}%</p></div>
                <div className="card"><p className="text-xs text-slate-500">Pass Rate</p><p className="text-2xl font-bold text-primary-600">{summary.passRate}%</p></div>
              </div>
            )}

            <div className="card">
              <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Users size={16} /> Students</h3>
              {students.length === 0 ? (
                <p className="text-slate-500 text-sm text-center py-8">No students have taken this exam yet.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="bg-slate-100"><th className="p-3 text-left font-semibold text-slate-600">Student</th><th className="p-3 text-center font-semibold text-slate-600">Attempts</th><th className="p-3 text-center font-semibold text-slate-600">Best Score</th><th className="p-3 text-center font-semibold text-slate-600">Average</th><th className="p-3 text-center font-semibold text-slate-600">Latest</th><th className="p-3 text-center font-semibold text-slate-600">Action</th></tr></thead>
        ...
        {view === 'analytics' && selectedStudent && (
          <>
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-900">{selectedStudent.student.first_name} {selectedStudent.student.last_name}</h2>
                <p className="text-sm text-slate-500">{selectedExam.title} — Detailed Report</p>
              </div>
              <a
            ...
            {studentAnalytics && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="card"><p className="text-xs text-slate-500">Total Attempts</p><p className="text-2xl font-bold">{studentAnalytics.total_attempts}</p></div>
                <div className="card"><p className="text-xs text-slate-500">Best Score</p><p className={`text-2xl font-bold ${getGradeColor(studentAnalytics.best_score)}`}>{studentAnalytics.best_score}%</p></div>
                <div className="card"><p className="text-xs text-slate-500">Average</p><p className="text-2xl font-bold text-blue-600">{studentAnalytics.average_score}%</p></div>
                <div className="card"><p className="text-xs text-slate-500">Latest</p><p className={`text-2xl font-bold ${getGradeColor(studentAnalytics.latest_score)}`}>{studentAnalytics.latest_score}%</p></div>
              </div>
            )}
            ...
            {studentAnalytics?.weakest_subjects?.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                  <p className="text-xs font-bold text-red-700 uppercase tracking-wider mb-2">Weakest Subjects</p>
                  {studentAnalytics.weakest_subjects.map((s: string) => <p key={s} className="text-sm text-red-800">• {s}</p>)}
                </div>
                <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                  <p className="text-xs font-bold text-green-700 uppercase tracking-wider mb-2">Strongest Subjects</p>
            ...
            {studentAnalytics?.recommended_pathway && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <p className="text-xs font-bold text-blue-700 uppercase tracking-wider mb-2">Recommended Pathway</p>
                <p className="text-sm font-bold text-primary-700">{studentAnalytics.recommended_pathway}</p>
                <p className="text-sm text-slate-700">{studentAnalytics.pathway_reasoning}</p>
              </div>
            )}

            {/* Attempt History */}
            <div className="card">
              <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><BarChart3 size={16} /> Attempt History</h3>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {studentAttempts.map((a: any) => (
                      <tr key={a.id}>
                        ...
                        <td className="p-2 text-center text-slate-500">{a.time_taken_seconds ? `${Math.floor(a.time_taken_seconds / 60)}m` : '—'}</td>
                        <td className="p-2 text-center text-slate-500">{a.completed_at ? new Date(a.completed_at).toLocaleDateString() : '—'}</td>
                        <td className="p-2 text-center">
                          <a href={`/student/mock-exams/report/${a.id}`} target="_blank" className="text-primary-600 hover:underline text-xs">View</a>
                        </td>
                      </tr>
                    ))}
                  </tbody>
