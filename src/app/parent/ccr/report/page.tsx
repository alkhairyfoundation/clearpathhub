...
function ParentCcrReportContent() {
    ...
    return (
      <DashboardLayout title="CCR Report" subtitle="Child Review Report">
        <div className="text-center py-20">
          <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400">Please select a child from the CCR dashboard.</p>
  ...
  if (!sgi) {
    return (
      <DashboardLayout title="CCR Report" subtitle="Child Review Report">
        <div className="text-center py-20 text-slate-500 dark:text-slate-400 dark:text-slate-400">No report available yet. Complete the questionnaires first.</div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout
      ...
      subtitle={`Student Growth Index: ${sgi.overall}/100`}><div className="flex items-center justify-between mb-4">
        <Link href="/parent/ccr" className="text-sm text-primary-600 dark:text-primary-400 dark:text-primary-400 hover:underline flex items-center gap-1">
          <ArrowLeft className="w-4 h-4" /> Back
        </Link>
        <button
          onClick={async () => {
