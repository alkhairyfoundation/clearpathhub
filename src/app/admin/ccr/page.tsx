    ...
    <DashboardLayout
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h3 className="font-semibold text-slate-800 mb-4">By Respondent Type</h3>
              ...
              return (
                <div key={type}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-slate-500">{data.submitted}/{data.total}</span>
                  </div>
          ...
          {recentSubmissions.length === 0 ? (
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {recentSubmissions.map((sub, i) => (
                <div key={sub.id || i} className="flex items-center justify-between text-sm p-2 bg-slate-50 rounded">
                  <span className="text-slate-600 font-mono text-xs">{sub.student_id?.substring(0, 8)}...</span>
                  <span className="capitalize text-slate-700">{sub.respondent_type}</span>
