...
export default function AdminEvaluationPage() {
  ...
  const taskTypeIcons: Record<string, { bg: string; color: string }> = {
    reading: { bg: 'bg-primary-100 dark:bg-primary-900/30 dark:bg-primary-900/30', color: 'text-primary-600 dark:text-primary-400 dark:text-primary-400' }, study: { bg: 'bg-purple-100 dark:bg-purple-900/30 dark:bg-purple-900/30', color: 'text-purple-600 dark:text-purple-400 dark:text-purple-400' },
    project: { bg: 'bg-green-100 dark:bg-green-900/30 dark:bg-green-900/30', color: 'text-green-600 dark:text-green-400 dark:text-green-400' }, research: { bg: 'bg-amber-100 dark:bg-amber-900/30 dark:bg-amber-900/30', color: 'text-amber-600 dark:text-amber-400 dark:text-amber-400' },
  };
  ...
  return (
    <DashboardLayout title="Teacher Evaluation" subtitle="Assign tasks, evaluate performance, and track progress">
      ...
      <div className="card">
        ...
        {activeTab === 'tasks' && (
          <>
            <div className="relative mb-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 dark:text-slate-500" size={18} /><input type="text" placeholder="Search tasks..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="input pl-10" /></div>
            ...
            filteredTasks.length === 0 ? <div className="text-center py-16"><BookOpen className="mx-auto text-slate-300 mb-4" size={48} /><p className="font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400">No tasks assigned yet</p></div> :
            <div className="space-y-3">
              {filteredTasks.map(task => {
                const typeStyle = taskTypeIcons[task.task_type] || taskTypeIcons.reading;
                return (
                  <div key={task.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 dark:border-slate-700">
                    <div className="flex items-center gap-4 mb-3 sm:mb-0">
                      ...
                      <div><p className="font-semibold text-slate-900 dark:text-white dark:text-white">{task.title}</p><p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">{task.teacher?.first_name} {task.teacher?.last_name}{task.due_date ? ` • Due ${new Date(task.due_date).toLocaleDateString()}` : ''}</p></div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${task.status === 'graded' ? 'bg-green-100 dark:bg-green-900/30 dark:bg-green-900/30 text-green-700 dark:text-green-300 dark:text-green-300' : 'bg-amber-100 dark:bg-amber-900/30 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 dark:text-amber-300'}`}>{task.status}</span>
                      {task.status === 'pending' && (
                        <div className="flex gap-1">
                          {[70, 80, 90, 100].map(grade => (
                            <button key={grade} onClick={() => handleGradeTask(task.id, grade)} className="px-2 py-1 text-xs bg-white border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded hover:bg-primary-50 dark:bg-primary-900/20 dark:bg-primary-900/20 hover:border-primary-300">{grade}</button>
                          ))}
                        </div>
                      )}
                      {task.status === 'graded' && <span className="font-bold text-primary-600 dark:text-primary-400 dark:text-primary-400">{task.admin_grade}/100</span>}
                    </div>
                  </div>
                );
              })}
        ...
        {activeTab === 'evaluations' && (
          evaluations.length === 0 ? <div className="text-center py-16"><Star className="mx-auto text-slate-300 mb-4" size={48} /><p className="font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400">No evaluations yet</p></div> :
          <div className="space-y-3">
            {evaluations.map(eval_ => (
              <div key={eval_.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 dark:bg-slate-800 rounded-xl">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-purple-100 dark:bg-purple-900/30 dark:bg-purple-900/30 rounded-full flex items-center justify-center"><Award size={20} className="text-purple-600 dark:text-purple-400 dark:text-purple-400" /></div>
                  <div><p className="font-semibold text-slate-900 dark:text-white dark:text-white">{eval_.title}</p><p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">{eval_.teacher?.first_name} {eval_.teacher?.last_name} • {new Date(eval_.created_at).toLocaleDateString()}</p></div>
                </div>
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${eval_.status === 'completed' ? 'bg-green-100 dark:bg-green-900/30 dark:bg-green-900/30 text-green-700 dark:text-green-300 dark:text-green-300' : 'bg-amber-100 dark:bg-amber-900/30 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 dark:text-amber-300'}`}>{eval_.status}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {activeTab === 'leaderboard' && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/30 dark:bg-amber-900/30 rounded-lg flex items-center justify-center"><Trophy className="text-amber-600 dark:text-amber-400 dark:text-amber-400" size={20} /></div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white dark:text-white">Teacher Performance Rankings</h3>
            </div>
                ...
                <div className="flex items-center gap-1">
                  {teacher.avgGrade > 0 ? (
                    <>
                      ...
                      <span className="ml-2 font-bold text-slate-900 dark:text-white dark:text-white">{teacher.avgGrade}</span>
                    </>
                  ) : <span className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-500">Not graded</span>}
                </div>
              </div>
            ))}
          </div>
