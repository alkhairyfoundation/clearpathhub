...
export default function StudentCcrReport() {
  ...
  if (!sgi) {
    return (
      <DashboardLayout title="My CCR Report" subtitle="Child Review Assessment Results">
        <div className="text-center py-20 text-slate-500">No report available yet. Complete your questionnaire first.</div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout
      ...
      {sgi.observationGaps.length > 0 && (
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl">
          <h4 className="font-semibold text-amber-700 flex items-center gap-2 mb-2"><TrendingUp className="w-4 h-4" /> Observation Gaps</h4>
          {sgi.observationGaps.map((gap, i) => <p key={i} className="text-sm text-amber-600">{gap}</p>)}
        </div>
      )}
