...
export default function WeeklyGoalsPage() {
  ...
  const dimensionMeta = [
    { key: 'academic', label: 'Academic', color: 'border-l-blue-500 bg-blue-50', icon: <Target size={18} className="text-blue-600" /> },
    { key: 'islamic', label: 'Islamic Character', color: 'border-l-emerald-500 bg-emerald-50', icon: <Target size={18} className="text-emerald-600" /> },
    { key: 'skills', label: 'Life Skills', color: 'border-l-purple-500 bg-purple-50', icon: <Target size={18} className="text-purple-600" /> },
  ];
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 mt-1">Derived from your daily achievements</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="card"><div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><Target size={16} /> Total Goals</div><p className="text-2xl font-bold text-slate-900">{stats.total}</p></div>
          <div className="card"><div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><CheckCircle size={16} className="text-emerald-500" /> Completed</div><p className="text-2xl font-bold text-emerald-600">{stats.completed}</p></div>
          <div className="card"><div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><Clock size={16} className="text-amber-500" /> In Progress</div><p className="text-2xl font-bold text-amber-600">{stats.active}</p></div>
          <div className="card"><div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><AlertCircle size={16} className="text-red-500" /> Missed</div><p className="text-2xl font-bold text-red-600">{stats.missed}</p></div>
        </div>
        ...
        <div className="space-y-4">
              ...
              <div key={dim.key} className="card overflow-hidden">
                <div className={`border-l-4 ${dim.color} p-4`}>
                  ...
                  <div className="space-y-2">
                    {dimGoals.map(goal => (
                      <div key={goal.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-100">
          ...
          {goals.length === 0 && (
            <div className="card text-center py-12">
              ...
              <p className="font-medium text-slate-500">No weekly goals yet</p>
              <p className="text-sm text-slate-400 mt-1 mb-4">Complete daily practice to auto-generate weekly goals</p>
