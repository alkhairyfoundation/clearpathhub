      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-white dark:text-white">XP & Levels</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Every action earns you experience points</p>
            </div>
          </div>
        </div>
        ...
        {/* Level Card */}
        {levelData && (
          <div className="card bg-gradient-to-r from-amber-50 to-yellow-50 border-amber-200 dark:border-amber-900/40 dark:border-amber-900/40">
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <Trophy size={20} className="text-amber-600 dark:text-amber-400 dark:text-amber-400" />
                  <span className="text-3xl font-bold text-slate-900 dark:text-white dark:text-white">Level {levelData.level}</span>
                </div>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 dark:text-slate-400 mt-2">
                  Total XP: <span className="font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">{levelData.total_xp}</span>
                </p>
              </div>
              <div className="text-right">
                <div className="w-16 h-16 bg-amber-200 rounded-full flex items-center justify-center">
                  <span className="text-2xl font-bold text-amber-700 dark:text-amber-300 dark:text-amber-300">{levelData.level}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <div className="card py-2 px-3 text-center">
            <Zap size={16} className="text-amber-500 dark:text-amber-400 dark:text-amber-400 mx-auto mb-1" />
            <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Today</p>
            <p className="text-lg font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">+{xpToday}</p>
          </div>
          <div className="card py-2 px-3 text-center">
            <TrendingUp size={16} className="text-primary-500 mx-auto mb-1" />
            <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">This Week</p>
            <p className="text-lg font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">+{xpThisWeek}</p>
          </div>
          <div className="card py-2 px-3 text-center">
            ...
            <p className="text-lg font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">{streakData?.current_streak || 0} days</p>
          </div>
          <div className="card py-2 px-3 text-center">
            <Award size={16} className="text-purple-500 dark:text-purple-400 dark:text-purple-400 mx-auto mb-1" />
            <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Best Streak</p>
            <p className="text-lg font-bold text-slate-800 dark:text-slate-200 dark:text-slate-200">{streakData?.longest_streak || 0} days</p>
          </div>
        </div>
        ...
        {xpTypes.length > 0 && (
          <div className="card">
            <h3 className="font-bold text-slate-900 dark:text-white dark:text-white mb-3">XP Breakdown</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {xpTypes.map(type => {
                const total = transactions.filter(t => t.xp_type === type).reduce((s, t) => s + t.xp_amount, 0);
                return (
                  <div key={type} className="bg-slate-50 dark:bg-slate-800 dark:bg-slate-800 rounded-lg p-2 text-center">
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 uppercase">{type.replace(/_/g, ' ')}</p>
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-300">+{total}</p>
                  </div>
                );
              })}
            </div>
        ...
        <div className="flex items-center gap-2">
          <Filter size={14} className="text-slate-400 dark:text-slate-500 dark:text-slate-500" />
        ...
        <div className="card p-0 overflow-hidden">
          {filteredTxs.length === 0 && (
            <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">
              ...
              <div key={tx.id} className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 dark:border-slate-700 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:bg-slate-800 dark:bg-slate-800">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    ...
                    <Zap size={14} className={isPositive ? 'text-emerald-600 dark:text-emerald-400 dark:text-emerald-400' : 'text-red-500 dark:text-red-400 dark:text-red-400'} />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-500">
                      ...
                      {isMultiplied && (
                        <>
                          <span>•</span>
                          <span className="text-amber-500 dark:text-amber-400 dark:text-amber-400 font-medium">x{tx.multiplier}</span>
                        </>
                      )}
                    </div>
                  </div>
                </div>
                <span className={`text-sm font-bold ${isPositive ? 'text-emerald-600 dark:text-emerald-400 dark:text-emerald-400' : 'text-red-500 dark:text-red-400 dark:text-red-400'}`}>
        ...
        {/* XP Sources Info */}
        <div className="card bg-gradient-to-r from-primary-50 to-amber-50 border-primary-200 dark:border-primary-900/40 dark:border-primary-900/40">
          <h3 className="font-bold text-slate-900 dark:text-white dark:text-white mb-2 flex items-center gap-2">
            <Star size={16} className="text-primary-600 dark:text-primary-400 dark:text-primary-400" />
            How to Earn XP
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
            {[
              ...
              <div key={i} className="flex items-center justify-between bg-white/60 rounded-lg px-2.5 py-1.5">
                <span className="text-slate-600 dark:text-slate-400 dark:text-slate-400">{item.action}</span>
                <span className="font-bold text-primary-600 dark:text-primary-400 dark:text-primary-400">{item.xp}</span>
              </div>
            ))}
          </div>
        </div>
