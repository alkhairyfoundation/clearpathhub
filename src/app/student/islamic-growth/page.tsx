    ...
    <DashboardLayout title="Islamic Growth" subtitle="Track your Islamic character development">
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Islamic Growth</h1>
            <p className="text-slate-500 mt-1">Salah, Quran, Adab, and character tracking</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="card">
            <div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><Moon size={16} /> Days Tracked</div>
            <p className="text-2xl font-bold text-slate-900">{stats.totalDays}</p>
          </div>
          <div className="card">
            <div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><Star size={16} /> Avg Adab</div>
            <p className="text-2xl font-bold text-emerald-600">{stats.avgAdab}/5</p>
          </div>
          <div className="card">
            <div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><Book size={16} /> Ayahs Memorized</div>
            <p className="text-2xl font-bold text-amber-600">{stats.memorizedAyahs}</p>
          </div>
          <div className="card">
            <div className="flex items-center gap-2 text-sm text-slate-500 mb-1"><CheckCircle size={16} /> Perfect Days</div>
            <p className="text-2xl font-bold text-green-600">{stats.perfectDays}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                  <Moon size={14} className="text-emerald-600" /> Salah (Prayers)
                </h3>
                <div className="space-y-2">
                  {salahNames.map(s => (
                    <label key={s.key} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg cursor-pointer hover:bg-slate-100">
                      <div>
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                  <Book size={14} className="text-amber-600" /> Quran
                </h3>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-500">Ayahs Memorized</label>
                  ...
                  <div>
                    <label className="text-xs text-slate-500">Ayahs Revised</label>
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                  <Heart size={14} className="text-rose-600" /> Adab & Character
                </h3>
                <div className="space-y-3">
                  ...
                  <label className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg cursor-pointer">
                    ...
                    <span className="text-sm text-slate-700">Completed morning/evening adhkar</span>
                  </label>
                  <div>
                    <label className="text-xs text-slate-500">Charity / Good Deed</label>
          ...
          <div className="space-y-6">
            <div className="card">
              ...
              {history.length === 0 ? (
                <p className="text-sm text-slate-400 text-center py-4">Start tracking your Islamic growth</p>
              ) : (
                <div className="space-y-2">
                  {history.slice(0, 10).map(day => {
                    ...
                    return (
                      <div key={day.id} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg">
                        <div className="flex items-center gap-2">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                            isPerfect ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'
                          }`}>
                        ...
                        <div className="flex items-center gap-1">
                          {day.adab_rating != null && (
                            <span className="text-xs text-amber-600 font-medium">{day.adab_rating}/5</span>
                          )}
                        </div>
                      </div>
                    );
                ...
                {['Fajr', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'].map((name, i) => {
                  ...
                  return (
                    <div key={name} className="text-center">
                      ...
                      <div className="w-full bg-slate-100 rounded-full h-16 flex flex-col justify-end overflow-hidden">
